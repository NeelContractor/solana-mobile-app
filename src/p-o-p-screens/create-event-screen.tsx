...
export default function CreateEventScreen() {
    const { createEvent, allEvents } = usePOPProgram()
    ...
    async function handleCreateEvent() {
        ...
        try {
            ...
            // 2. Get connected wallet pubkey
            setStatusMsg('Connecting wallet...')
            const publicKey = await wallet.connect()

            setStatusMsg('Please approve in your wallet...')

            // 3. Call mutation — handles build + sign + send + confirm internally
            const { signature } = await createEvent.mutateAsync({
                organizerPubkey: publicKey.publicKey,
                name: form.name.trim(),
                lat: latitude,
                lng: longitude,
...
const styles = StyleSheet.create({
  scroll: { flex: 1, backgroundColor: '#0f0f23' },
  container: { padding: 24, paddingBottom: 48 },
  title: { fontSize: 28, fontWeight: 'bold', color: '#9945FF', marginBottom: 6 },
  subtitle: { fontSize: 14, color: '#888', marginBottom: 20, lineHeight: 20 },
  banner: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 10, marginBottom: 24 },
  bannerText: { color: '#fff', fontSize: 13, flex: 1 },
  field: { marginBottom: 20 },
  label: { color: '#ccc', fontSize: 14, fontWeight: '600', marginBottom: 8 },
  hint: { color: '#555', fontSize: 11, marginTop: 4, textAlign: 'right' },
  input: {
  ...
  presetActive: { backgroundColor: '#9945FF', borderColor: '#9945FF' },
  ...
  infoTitle: { color: '#9945FF', fontWeight: '700', fontSize: 14, marginBottom: 8 },
  infoText: { color: '#aaa', fontSize: 13, lineHeight: 22 },
  infoHighlight: { color: '#14F195', fontWeight: '700' },
  button: { backgroundColor: '#9945FF', paddingVertical: 16, borderRadius: 12, alignItems: 'center' },
  ...
  successTitle: { color: '#14F195', fontSize: 22, fontWeight: 'bold', marginBottom: 12 },
  successName: { color: '#fff', fontSize: 18, fontWeight: '600', marginBottom: 12 },
  successTx: { color: '#888', fontSize: 11, marginBottom: 24, width: '100%', textAlign: 'center' },
  resetButton: { backgroundColor: '#9945FF', paddingVertical: 12, paddingHorizontal: 32, borderRadius: 10 },
